import React, { memo } from 'react';
import { motion, AnimatePresence } from 'framer-motion';

interface IProps {
  isModalVisible: boolean;
  desc: string;
  handleHideModal: () => void;
}

function ModalInfo(props: IProps) {
  return (
    <div className="overflow-hidden">
      <AnimatePresence>
        {props.isModalVisible ? (
          <div className="overflow-hidden">
            <motion.div
              className="absolute left-[38%] top-[15%] z-50 flex h-1/2 w-1/4 items-center overflow-hidden"
              style={{ overflow: 'hidden' }}
              initial={{ x: -700 }}
              animate={{ x: 0 }}
              exit={{ opacity: 0 }}
            >
              <div className="h-1/2 w-full ">
                <div className="flex w-full flex-col rounded-xl border-0 bg-[#1F2937] shadow-lg">
                  <div className="flex items-center justify-between border-b border-solid border-slate-600 px-5 pb-1 pt-2 text-xl">
                    <h1 className="font-semibold">Input Field Description</h1>
                    <button
                      className="opacity-4 float-right border-0 bg-transparent p-1 text-3xl font-semibold leading-none text-slate-200 outline-none focus:outline-none"
                      onClick={props.handleHideModal}
                    >
                      ×
                    </button>
                  </div>
                  <div className="rounded-xl p-5 pb-[100px]">
                    <p className="text-md text-slate-200">{props.desc}</p>
                  </div>
                </div>
              </div>
            </motion.div>
            <motion.div
              className="fixed inset-0 z-40 overflow-hidden bg-black"
              initial={{ opacity: 0 }}
              animate={{ opacity: 0.25 }}
              exit={{ opacity: 0 }}
              onClick={props.handleHideModal}
            />
          </div>
        ) : null}
      </AnimatePresence>
    </div>
  );
}
export default memo(ModalInfo);
